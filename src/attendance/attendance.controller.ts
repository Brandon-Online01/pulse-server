...
export class AttendanceController {
	...
	@ApiOperation({
		...
		description: 'Retrieves a comprehensive list of all attendance check-ins with full user profiles, branch information, and verification details',
	})
	@ApiOkResponse({
				...
				checkIns: {
					...
					items: {
						...
						properties: {
							...
							status: { type: 'string', enum: ['PRESENT', 'COMPLETED', 'ON_BREAK'], example: 'COMPLETED' },
							checkIn: { type: 'string', format: 'date-time', example: '2024-03-01T09:00:00Z' },
							checkOut: { type: 'string', format: 'date-time', example: '2024-03-01T17:30:00Z', nullable: true },
							...
							owner: {
								...
								properties: {
									...
									accessLevel: { type: 'string', enum: ['ADMIN', 'MANAGER', 'USER', 'TECHNICIAN'], example: 'USER' },
									createdAt: { type: 'string', format: 'date-time' },
									updatedAt: { type: 'string', format: 'date-time' },
									branch: {
										type: 'object',
	...
	@ApiOperation({
		...
		description: 'Retrieves comprehensive attendance records for a specific date with full user profiles, branch information, and verification details ordered by check-in time',
	})
	@ApiParam({ 
		name: 'date', 
	...
	@ApiOperation({
		...
		description: 'Retrieves comprehensive attendance records for a specific user with full user profile, branch information, and verification details ordered by check-in time',
	})
	@ApiParam({ 
		name: 'ref', 
			...
			properties: {
				...
				user: {
					...
					properties: {
						...
						role: { type: 'string', example: 'employee' },
						status: { type: 'string', example: 'active' },
						accessLevel: { type: 'string', enum: ['ADMIN', 'MANAGER', 'USER', 'TECHNICIAN', 'HR'], example: 'USER' },
						createdAt: { type: 'string', format: 'date-time' },
						updatedAt: { type: 'string', format: 'date-time' },
						branch: {
							type: 'object',
	...
	@ApiOperation({
		...
		description: 'Retrieves the latest attendance status for a specific user with complete user profile and shift information including timing details and next available action',
	})
	@ApiParam({ 
		name: 'ref', 
	...
	@ApiOperation({
		...
		description: 'Retrieves comprehensive attendance records for a specific branch with full user profiles, branch information, and summary statistics including unique user count',
	})
	@ApiParam({ 
		name: 'ref', 
	...
	@ApiOperation({
		...
		description: 'Retrieves detailed work and break time statistics for a specific user for a given day (defaults to today). Returns time in milliseconds for precise calculations.',
	})
	@ApiParam({ 
		name: 'uid', 
	...
	@ApiOperation({
		...
		description: 'Retrieves detailed attendance analytics for a specific user including historical data, productivity insights, break patterns, timing analysis, and performance metrics across different time periods',
	})
	@ApiParam({ 
		name: 'uid', 
				...
				metrics: {
					...
					properties: {
						...
						totalHours: {
							...
							properties: {
								allTime: { type: 'number', example: 320.5, description: 'Total hours worked since first attendance' },
								thisMonth: { type: 'number', example: 160.0, description: 'Hours worked in current month' },
								thisWeek: { type: 'number', example: 40.0, description: 'Hours worked in current week' },
						...
						totalShifts: {
							...
							properties: {
								allTime: { type: 'number', example: 45, description: 'Total shifts since first attendance' },
								thisMonth: { type: 'number', example: 20, description: 'Shifts worked in current month' },
						...
						breakAnalytics: {
							...
							properties: {
								totalBreakTime: {
									...
									properties: {
										allTime: { type: 'number', example: 450, description: 'Total break minutes since first attendance' },
										thisMonth: { type: 'number', example: 240, description: 'Break minutes in current month' },
										thisWeek: { type: 'number', example: 75, description: 'Break minutes in current week' },
						...
						productivityInsights: {
							...
							properties: {
								workEfficiencyScore: { 
									type: 'number', 
									example: 88,
									description: 'Work efficiency percentage (work time vs total time including breaks)'
								},
								shiftCompletionRate: { 
									type: 'number', 
									example: 95,
	...
	@ApiOperation({
		...
		description: 'Generates comprehensive attendance report including individual user metrics and organization-level analytics',
	})
	...
	@ApiQuery({ name: 'includeUserDetails', required: false, type: 'boolean', description: 'Include individual user breakdowns' })
	@ApiOkResponse({
	...
	getOrganizationReport(
		@Query() queryDto: OrganizationReportQueryDto,
