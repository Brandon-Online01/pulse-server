...
import { TimeCalculatorUtil, WorkSession } from '../utils/time-calculator.util';
...
export class AttendanceCalculatorService {
  ...
  calculateAverageTimes(attendanceRecords: Attendance[]): {
    ...
    const averageShiftDuration = shiftDurations.length > 0
    ...
    const averageBreakDuration = breakDurations.length > 0
      ? TimeCalculatorUtil.roundToHours(
  ...
  async calculateProductivityMetrics(
    ...
    for (const record of attendanceRecords) {
      ...
      const punctuality = await this.calculatePunctuality(
        record.checkIn,
        record.checkOut,
        organizationId
      );
    ...
    // Calculate metrics
    const punctualityScore = attendanceRecords.length > 0
    ...
    const workEfficiencyScore = totalWorkMinutes > 0
      ? TimeCalculatorUtil.calculateEfficiency(
          totalWorkMinutes - totalBreakMinutes,
          totalWorkMinutes
        )
      : 0;

    const shiftCompletionRate = attendanceRecords.length > 0
