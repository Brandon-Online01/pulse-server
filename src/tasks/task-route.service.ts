...
import { Location, RouteStop, OptimizedRoute } from './interfaces/route.interface';
  ...
  private async planRouteForTask(task: Task, retryCount = 0) {
    ...
    for (const assignee of assignees) {
      try {
        ...
        if (!user?.branch) {
          console.warn(`No branch found for user ${assignee.uid}`);
          continue;
        }
        ...
        if (!branch?.address?.latitude || !branch?.address?.longitude) {
          console.warn(`Invalid branch location for branch ${branch?.uid}`);
          continue;
        }
        ...
        if (destinations.length === 0) {
          console.warn(`No valid client locations found for task ${task.uid}`);
          continue;
        }
        ...
        try {
          ...
          console.error(`Failed to optimize route for task ${task.uid}:`, error);
          if (retryCount < MAX_RETRIES) {
            console.log(`Retrying route optimization (attempt ${retryCount + 1}/${MAX_RETRIES})`);
